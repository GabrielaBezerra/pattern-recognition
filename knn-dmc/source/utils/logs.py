class Logs:
    ...
    def model(self, model, database):
        print(f"\n\033[1;33m# {model} Experiment in {database} \033[0m")

    def database(self, database):
        print(f"\n\033[1;32m# {database} database \033[0m")

    def realization_details(self, i, model, split, train, test, metrics):
        if self.verbose:
            print(f"\n\033[1;31m{model.name}\033[0m \033[1;32m#{i} Realization\033[0m")
            self.print_split_details(split, train, test)
            self.print_metrics_for_realization(metrics)

    def print_split_details(self, split, train, test):
        if self.verbose:
    ...
    def print_metrics_for_realization(self, metrics):
    ...
    def experiment_results(self, final_metrics):
        if self.verbose:
