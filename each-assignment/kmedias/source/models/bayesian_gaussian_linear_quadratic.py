...
class BayesianGaussianLinear:
    ...
    def __init__(self) -> None:
        self.name = "BayesianGaussianLinear"
    ...
    def fit(self, train):
        ...
        self.shared_cov = np.zeros((train.shape[1] - 1, train.shape[1] - 1))

        # Estimating priors, means, and the shared covariance matrix
        for i, c in enumerate(self.classes):
            data = train[train[:, -1] == c][:, :-1]
            self.class_priors[i] = len(data) / len(train)
            self.class_means[i] = np.mean(data, axis=0)
            self.shared_cov += np.cov(data.T) * (len(data) - 1)

        # Normalizing by the total number of samples to get the shared covariance
        self.shared_cov /= (len(train) - len(self.classes))
    ...
    def predict(self, test, has_labels=True):
        ...
        for newData in test:
            ...
            for i, _ in enumerate(self.classes):
                ...
                # Using shared covariance matrix for likelihood computation
                likelihood = self._multivariate_gaussian(data, mean, self.shared_cov)
    ...
    def __copy__(self):
        classifier = BayesianGaussianLinear()
        ...
        classifier.shared_cov = self.shared_cov.copy()
        return classifier


class BayesianGaussianQuadratic:
    classes: np.ndarray
    class_priors: np.ndarray
    class_means: np.ndarray
    ...
    def __init__(self) -> None:
        self.name = "BayesianGaussianQuadratic"
    ...
    def __copy__(self):
        classifier = BayesianGaussianQuadratic()
        classifier.classes = self.classes.copy()
        classifier.class_priors = self.class_priors.copy()
        classifier.class_means = self.class_means.copy()
